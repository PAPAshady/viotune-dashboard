...
function PlaylistSongsSheet({ playlist }) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenSheetChange}>
      ...
      <SheetContent className="w-full sm:max-w-xl! lg:max-w-2xl!">
        {tab === 'details' ? (
          <DetailsView playlist={playlist} open={open} onAddSongClick={() => setTab('add-song')} />
        ) : (
          <AddSongView playlist={playlist} onBack={() => setTab('details')} />
        )}
      </SheetContent>
    </Sheet>
  );
...
export default PlaylistSongsSheet;
